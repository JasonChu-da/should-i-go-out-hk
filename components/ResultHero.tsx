  ...
  return (
    <section className="result-hero" data-verdict={result.verdict} aria-labelledby="result-title">
      <p className="sr-only" aria-live="polite" aria-atomic="true">
        {MODE_COPY[mode]}。{scoreLabel}。{result.isLimited ? "資料有限" : "資料齊備"}。{result.verdictLabel}。{result.summary}
      </p>
      <h2 className="verdict" id="result-title" tabIndex={-1}>
        {result.verdictLabel}
      </h2>

      <div className="outlook-index">
        <span className="verdict-symbol" aria-hidden="true">
          <AppIcon name={VERDICT_ICONS[result.verdict]} />
        </span>
        <span className="outlook-index-copy">
          <span>外出指數</span>
          <strong>{result.score === null ? "暫未能評分" : `${result.score}/10`}</strong>
        </span>
        {result.score !== null ? (
          <div
