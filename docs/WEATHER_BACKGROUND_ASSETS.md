...
- 硬性驗收門檻是 42 個唯一且存在的 WebP 路徑、原生 desktop 16:9／mobile 9:16 構圖、可讀文字安全區、正確場景語義及 `<picture>` responsive 選圖；不是指定的最小像素尺寸。
- 1792×1024（desktop）及 1024×1792（mobile）是後續提升細節的設計目標，不是目前版本的硬性驗收或上線門檻。
- WebP：quality 92、smart chroma subsampling；轉檔沒有 resize。
- 42 個路徑均唯一且存在，總大小約 13.63 MiB；每次只下載當前場景及 viewport 所選方向。
- 內建生成器本輪實際原生輸出：desktop 1659–1660×948，mobile 941×1672。這低於上述設計目標，但通過目前硬性驗收；亦沒有用插值放大冒充原生輸出。
